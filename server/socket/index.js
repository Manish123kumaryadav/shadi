...
async function canSignalCall(conversationId, fromUserId, toUserId) {
  if (!conversationId || !toUserId || Number(fromUserId) === Number(toUserId)) return false;
...
export function registerSocketHandlers(io) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('call:offer', async ({ conversationId, toUserId, callId, offer }) => {
      if (!(await canSignalCall(conversationId, socket.user.id, toUserId))) return;
    ...
    socket.on('call:answer', async ({ conversationId, toUserId, callId, answer }) => {
      if (!(await canSignalCall(conversationId, socket.user.id, toUserId))) return;
    ...
    socket.on('call:ice-candidate', async ({ conversationId, toUserId, callId, candidate }) => {
      if (!(await canSignalCall(conversationId, socket.user.id, toUserId))) return;
